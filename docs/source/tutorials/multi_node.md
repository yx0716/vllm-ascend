...
Execute the following commands on each node in sequence. The results must all be `succeed` and the status must be `UP`:
...
Below is the example container setup command, which should be executed on **all nodes** (ensure to modify the container name `--name` and the image tag):
...
Set the `ASCEND_RT_VISIBLE_DEVICES` environment variable to specify the NPU devices to use. For Ray versions above 2.1, also set the `RAY_EXPERIMENTAL_NOSET_ASCEND_RT_VISIBLE_DEVICES` variable to avoid device recognition issues. The `--num-gpus` parameter defines the number of GPUs to be used on each node.
...
Once the cluster is started on multiple nodes, execute `ray status` and `ray list nodes` to verify the Ray cluster’s status. You should see the correct number of nodes and GPUs listed.
...
For example, with 16 NPUs across 2 nodes (8 GPUs per node), set the tensor parallel size to 8 and the pipeline parallel size to 2:
...
Alternatively, if you want to use only tensor parallelism, set the tensor parallel size to the total number of NPUs in the cluster. For example, with 16 GPUs across 2 nodes, set the tensor parallel size to 16:
```shell
Python -m vllm.entrypoints.openai.api_server \
       --model=”Deepseek/DeepSeek-V2-Lite-Chat” \
       --trust-remote-code \
